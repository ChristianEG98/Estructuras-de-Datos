...
bool esOperador(const std::string& str) {
    return (str == "+" || str == "-" || str == "*" || str == "/");
}

int calcularValor(const std::string& str, const int& iz, const int& dr) {
    int valor;
    if (str == "+") {
        valor = iz + dr;
    }
...
int evaluarExpresion(BinTree<std::string> arbol, MapTree<std::string, int> mapa) {
    stringstream ss, siz, sdr;
    int valor = 0, iz, dr;
    if (arbol.left().empty() && arbol.right().empty()) { //Cuando es una hoja y no es un numero
        if (!esNumero(arbol.root())) { //Si es una variable
            if (mapa.contains(arbol.root())) {
                return mapa.at(arbol.root());
            }
        }
        ...
        if (esOperador(arbol.root())) { //Cuando es un operador

            if (esOperador(arbol.right().root())) { //Si hay un operador en el subarbol derecho
                dr = evaluarExpresion(arbol.right(), mapa);
            }
            if (esOperador(arbol.left().root())) { //Si hay un operador en el subarbol izquierdo
                iz = evaluarExpresion(arbol.left(), mapa);
            }
            
            if (!esNumero(arbol.right().root())) { //Si el subarbol derecho es una variable
                if (mapa.contains(arbol.right().root())) {
                    dr = mapa.at(arbol.right().root());
                }
            }
            else { //Si el subarbol derecho es un numero
                sdr << arbol.right().root();
                sdr >> dr;
            }

            if (!esNumero(arbol.left().root())) { //Si el subarbol izquierdo es una variable
                if (mapa.contains(arbol.left().root())) {
                    iz = mapa.at(arbol.left().root());
                }
            }
            else { //Si el subarbol izquierdo es un numero
                siz << arbol.left().root();
                siz >> iz;
            }

            valor = calcularValor(arbol.root(), iz, dr);
        }
    }
    return valor;
}

//Coste cuadratico O(M*S) siendo M el numero de asignaciones que realiza y S el numero maximo de nodos
bool tratar_caso() {
    int num, valorExpresion;
    MapTree<std::string, int> mapaVariables;
    BinTree<std::string> arbolExpresion;
    cin >> num;
    if (num == 0) return false;
    for (int i = 0; i < num; i++) { //Coste cuadratico O(M*S) siendo M el numero de asignaciones que realiza y S el numero maximo de nodos
        arbolExpresion = parse(std::cin);
        valorExpresion = evaluarExpresion(arbolExpresion.right(), mapaVariables); 
        mapaVariables[arbolExpresion.left().root()] = valorExpresion;
    }
    for (auto it : mapaVariables) cout << it.key << " = " << it.value << endl; //Coste lineal O(N) siendo N el numero de variables
    cout << "---" << endl;
    return true;
}
