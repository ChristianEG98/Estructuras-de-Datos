...
//Coste O(n * log n) siendo n el numero de cartas de la baraja
bool tratar_caso() {
    int tam_jugadores, tam_baraja, carta;
    cin >> tam_jugadores;
    if (cin.eof()) return false;
    cin >> tam_baraja;
    vector<SetTree<int>> jugadores(tam_jugadores);
    int jugador_contador = 0;
    for (int i = 0; i < tam_baraja; i++) {
        if (jugador_contador == tam_jugadores) jugador_contador = 0;
        cin >> carta;
        if (jugadores[jugador_contador].contains(carta)) {
            jugadores[jugador_contador].erase(carta);
        }
        else {
            jugadores[jugador_contador].insert(carta);
        }
        jugador_contador++;
    }
    for (int i = 0; i < tam_jugadores; i++) {
        cout << "J" << i + 1 << ": " << jugadores[i] << endl;
    }
    cout << "---" << endl;
    return true;
}
